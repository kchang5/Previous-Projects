...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while current_place != beehive:
            bee_location = rANTdom_else_none(current_place.bees)
            if bee_location and self.max_range >= counter >=self.min_range:
                return bee_location
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 5
        lst_of_bees = list(self.place.bees)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem 6
        #bee_location = rANTdom_else_none(self.place.bees)
        if self.place.bees == []: #no bees
            return 
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        if self.is_true_queen == False:
        	...
        	while current != None:
        		if current.ant and current.ant not in self.buffed_ants:
        			current.ant.damage *= 2
        			self.buffed_ants.append(current.ant)
        		if current.ant and isinstance(current.ant.contained_ant, ContainerAnt) and current.ant.ant and current.ant.ant not in self.buffed_ants:
        			current.ant.ant.damage *= 2 
        			self.buffed_ants.append(current.ant.ant)      
